...
ALE가 라그랑지안과 Eulerian과 차별화되는 점은 메시의 속도( \widehat{v})가 입자(물질)의 속도($v$)와 독립적이라는 것입니다. 이 둘의 차이로 인한 물리량의 차이를 보정해주는 대류 속도(convection velocity)가 논문의 14번 식에 나타납니다.(- Donea, J. et al. (2004). Arbitrary Lagrangian-Eulerian Methods. *Encyclopedia of Computational Mechanics*.).
...
막이 변위 $w$만큼 움직이면, 내부 격자들이 겹치지 않도록 변위를 **라플라스 방정식**으로 도메인 전체에 부드럽게 전파합니다. 이는 각 노드가 3D 벡터장에서 어떤 방향으로 얼마나 움직이는지를 알려줍니다:
...
- $\Delta w$: 막의 증분 변위 (이번 스텝에서 새로 발생한 막 변위)
- $\hat{e}_x$: x방향 단위벡터. 수압을 받아 x방향으로 휜다고 가정
- $k$: **가변 강성(Variable Stiffness)** — 막에 가까울수록 크게 설정하여 좁은 격자의 뒤틀림을 방지합니다 만약 격자가 뒤틀리면 오류납니다(직접 겪어봄...):

$$k = \frac{1}{(|x - x_m| + \delta)^2}$$

$\delta$: 막과 노드의 거리가 0이 되는걸 방지
...
이 구조를 통해 유체 속도·압력, 막 변위, 메시 형상이 **물리적으로 일관되게 결합**되어 시뮬레이션될 것이라 예상됩니다. 지금 돌리고 있는데 잘 될런지...
